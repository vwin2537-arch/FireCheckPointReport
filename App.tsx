...
// นำเข้า Icon รายตัวจากเวอร์ชันที่เสถียร
import { 
  LockClosedIcon, 
  ChevronLeftIcon, 
  ...
  ChartBarIcon 
} from 'https://esm.sh/@heroicons/react@2.0.18/24/outline';

import { XCircleIcon as XCircleSolid } from 'https://esm.sh/@heroicons/react@2.0.18/24/solid';
...
const App: React.FC = () => {
  ...
  const executeSubmit = async () => {
    ...
    if (result.success) {
      setSuccessMessage(String(result.message)); // มั่นใจว่าเป็น string
  ...
  if (view === 'admin') return (
    <div className="min-h-screen bg-slate-50 p-4 pb-20 animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => loadDashboardData(state.date)} disabled={isLoadingDashboard} className="p-2 bg-white rounded-full shadow-sm">
            <ArrowPathRoundedSquareIcon className={`w-5 h-5 ${isLoadingDashboard ? 'animate-spin text-emerald-500' : 'text-slate-400'}`} />
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen pb-24 max-w-md mx-auto bg-slate-50 shadow-2xl relative overflow-x-hidden animate-fade-in">
      ...
      {successMessage ? (
        <div className="px-6 -mt-12">
           <div className="bg-white p-10 rounded-[3rem] shadow-2xl text-center border border-emerald-50 animate-fade-in">
        ...
        <main className="px-5 -mt-10 space-y-5 relative z-20">
          <div className="bg-white p-6 rounded-[2rem] shadow-md border-l-8 border-blue-500">
            <div className="flex items-center gap-2 mb-4 font-black text-[11px] text-blue-600 uppercase tracking-widest"><CalendarDaysIcon className="w-4 h-4"/> วันที่ปฏิบัติงาน</div>
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-md border-l-8 border-orange-500">
            <div className="flex items-center gap-2 mb-4 font-black text-[11px] text-orange-600 uppercase tracking-widest"><MapPinIcon className="w-4 h-4"/> จุดเฝ้าระวัง</div>
            <select value={state.pointId || ''} onChange={(e) => setState(prev => ({ ...prev, pointId: Number(e.target.value) }))} className="w-full p-4 bg-orange-50 border-none rounded-2xl font-bold text-slate-800 outline-none appearance-none">
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-md border-l-8 border-purple-500">
            <div className="flex items-center gap-2 mb-4 font-black text-[11px] text-purple-600 uppercase tracking-widest"><ClockIcon className="w-4 h-4"/> ช่วงเวลา</div>
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-md border-l-8 border-rose-500">
            <div className="flex justify-between mb-4 font-black text-[11px] uppercase tracking-widest">
              <div className="flex items-center gap-2 text-rose-600"><PhotoIcon className="w-4 h-4"/> รูปถ่าย</div>
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-md border-l-8 border-emerald-500">
            <div className="flex items-center gap-2 mb-4 font-black text-[11px] text-emerald-600 uppercase tracking-widest"><ChatBubbleLeftRightIcon className="w-4 h-4"/> เพิ่มเติม</div>
