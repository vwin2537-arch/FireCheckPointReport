...
// Direct Imports for stability on Vercel
import { 
  LockClosedIcon, 
  ChevronLeftIcon, 
...
const App: React.FC = () => {
  ...
  const checkDuplicate = () => {
    ...
    if (!point || !state.shift) return false;
  ...
  if (view === 'login') return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-slate-900 animate-in">
  ...
  if (view === 'admin') return (
    <div className="min-h-screen bg-slate-50 p-4 pb-20 animate-in">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <input type="date" value={state.date} onChange={(e) => setState(prev => ({...prev, date: e.target.value}))} className="text-[12px] font-bold border-none bg-white rounded-lg px-3 py-1.5 shadow-sm" />
          <button onClick={() => loadDashboardData(state.date)} disabled={isLoadingDashboard} className="p-2 bg-white rounded-full shadow-sm text-slate-400">
            <ArrowPathRoundedSquareIcon className={`w-5 h-5 ${isLoadingDashboard ? 'animate-spin text-emerald-500' : ''}`} />
          </button>
        </div>
      </div>
      <div className="bg-gradient-to-br from-emerald-800 to-emerald-950 text-white p-7 rounded-[2.5rem] shadow-xl mb-6 relative overflow-hidden">
        <div className="relative z-10">
          <h2 className="text-2xl font-black mb-4">สรุปการส่งรายงาน</h2>
        ...
        <FolderOpenIcon className="absolute -right-6 -bottom-6 w-40 h-40 text-white/5 rotate-12" />
      </div>
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden relative">
        {isLoadingDashboard && <div className="absolute inset-0 bg-white/70 backdrop-blur-[2px] z-20 flex items-center justify-center"><ArrowPathIcon className="w-10 h-10 text-emerald-600 animate-spin" /></div>}
        <div className="p-5 bg-slate-50 border-b text-[10px] font-black uppercase text-slate-400 flex justify-between">
          <span className="w-1/3">จุดเฝ้าระวัง</span>
          <div className="w-2/3 flex justify-around"><span>เช้า</span><span>กลาง</span><span>เย็น</span></div>
        </div>
        <div className="divide-y max-h-[60vh] overflow-y-auto custom-scrollbar">
          {WATCH_POINTS.map(point => {
            ...
            return (
              <div key={point.id} className="flex items-center p-5">
                ...
                <div className="w-2/3 flex justify-around">
                  {[Shift.MORNING, Shift.AFTERNOON, Shift.EVENING].map(s => (
                    status.some(f => f.shift === s) 
                      ? <CheckCircleIcon key={s} className="w-6 h-6 text-emerald-600" />
                      : <XCircleSolid key={s} className="w-6 h-6 text-rose-100" />
                  ))}
                </div>
              </div>
            );
  ...
  return (
    <div className="min-h-screen pb-24 max-w-md mx-auto bg-slate-50 shadow-2xl relative overflow-x-hidden font-sarabun animate-in">
      <input type="file" ref={fileInputRef} onChange={handleFileUpload} accept="image/*" multiple className="hidden" />

      {showConfirmOverlap && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-md"></div>
          <div className="bg-white w-full rounded-[2.5rem] p-8 relative z-10 shadow-2xl">
            <ExclamationTriangleIcon className="w-16 h-16 text-orange-500 mx-auto mb-4" />
            <h3 className="text-xl font-black text-center text-slate-800 mb-2">ส่งซ้ำ หรือ แก้ไข?</h3>
            <p className="text-center text-slate-500 text-sm mb-8">คุณเคยส่งรายงานช่วงนี้ไปแล้ว ต้องการแก้ไขใช่หรือไม่?</p>
            <div className="flex flex-col gap-3">
              <button onClick={executeSubmit} className="w-full py-4 bg-orange-600 text-white rounded-2xl font-bold">ยืนยันการส่งอีกครั้ง</button>
              <button onClick={() => setShowConfirmOverlap(false)} className="w-full py-4 bg-slate-100 text-slate-500 rounded-2xl font-bold">ยกเลิก</button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="relative bg-emerald-900 text-white pt-12 pb-20 px-6 rounded-b-[4rem] shadow-2xl">
        <div className="relative z-10 text-center space-y-3">
          <MapPinIcon className="w-12 h-12 text-yellow-400 mx-auto" />
          <h1 className="text-2xl font-black leading-tight">{APP_TITLE}</h1>
          <div className="px-5 py-1.5 bg-yellow-400 text-emerald-950 text-[11px] font-black rounded-full uppercase inline-block">อุทยานแห่งชาติเอราวัณ</div>
        </div>
      </header>
      ...
      {successMessage ? (
        <div className="px-6 -mt-12">
           <div className="bg-white p-10 rounded-[3rem] shadow-2xl text-center border border-emerald-50">
              <CheckCircleIcon className="w-20 h-20 text-emerald-600 mx-auto mb-6" />
              ...
              <div className="space-y-3">
                <button onClick={() => setSuccessMessage(null)} className="w-full py-4 bg-emerald-700 text-white rounded-2xl font-bold">กลับไปส่งใหม่</button>
                <button onClick={() => { setSuccessMessage(null); setView('login'); }} className="w-full py-4 bg-slate-50 text-slate-500 rounded-2xl font-bold border">ดู Dashboard</button>
              </div>
           </div>
        </div>
      ) : (
        <main className="px-5 -mt-10 space-y-5 relative z-20">
          <div className="bg-white p-6 rounded-[2rem] shadow-md border-l-8 border-blue-500">
            <div className="flex items-center gap-2 mb-4 font-black text-[11px] text-blue-600 uppercase"><CalendarDaysIcon className="w-4 h-4"/> วันที่ปฏิบัติงาน</div>
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-md border-l-8 border-orange-500">
            <div className="flex items-center gap-2 mb-4 font-black text-[11px] text-orange-600 uppercase"><MapPinIcon className="w-4 h-4"/> จุดเฝ้าระวัง</div>
            <select value={state.pointId || ''} onChange={(e) => setState(prev => ({ ...prev, pointId: Number(e.target.value) }))} className="w-full p-4 bg-orange-50 border-none rounded-2xl font-bold text-slate-800 outline-none">
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-md border-l-8 border-purple-500">
            <div className="flex items-center gap-2 mb-4 font-black text-[11px] text-purple-600 uppercase"><ClockIcon className="w-4 h-4"/> ช่วงเวลา</div>
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-md border-l-8 border-rose-500">
            <div className="flex justify-between mb-4 font-black text-[11px] uppercase">
              <div className="flex items-center gap-2 text-rose-600"><PhotoIcon className="w-4 h-4"/> รูปภาพหลักฐาน</div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {state.images.map((img, idx) => (
                <div key={idx} className="relative aspect-square rounded-xl overflow-hidden shadow-sm">
                  ...
                  <button onClick={() => setState(prev => ({...prev, images: prev.images.filter((_, i) => i !== idx)}))} className="absolute top-1 right-1 bg-rose-600 text-white p-1 rounded-full"><TrashIcon className="w-3 h-3"/></button>
                </div>
              ))}
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-md border-l-8 border-emerald-500">
            <div className="flex items-center gap-2 mb-4 font-black text-[11px] text-emerald-600 uppercase"><ChatBubbleLeftRightIcon className="w-4 h-4"/> บันทึกเพิ่มเติม</div>
            <textarea value={state.notes} onChange={(e) => setState(prev => ({ ...prev, notes: e.target.value }))} placeholder="รายละเอียดเหตุการณ์..." className="w-full p-4 bg-emerald-50/30 border-none rounded-2xl text-sm min-h-[100px] outline-none" />
          </div>

          <button onClick={handleSubmitAttempt} disabled={state.isSubmitting} className={`w-full py-5 rounded-[2rem] font-black text-xl flex items-center justify-center gap-2 shadow-xl ${state.isSubmitting ? 'bg-slate-300 text-slate-500' : 'bg-gradient-to-r from-emerald-600 to-emerald-800 text-white'}`}>
            {state.isSubmitting ? <><ArrowPathIcon className="w-6 h-6 animate-spin" /> กำลังส่ง...</> : 'ส่งรายงาน'}
          </button>

          <div className="pt-4 pb-10 flex justify-center">
            <button onClick={() => setView('login')} className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase tracking-widest py-3 px-8 bg-white border border-slate-100 rounded-full shadow-sm">
